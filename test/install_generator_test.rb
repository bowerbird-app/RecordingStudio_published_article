...
require "fileutils"
require "tmpdir"
require "generators/recording_studio_published_article/install/install_generator"

class InstallGeneratorTest < Minitest::Test
  INSTALL_TEMPLATE_PATH = File.expand_path(
    "../lib/generators/recording_studio_published_article/install/templates/INSTALL.md",
    __dir__
  )
  ...
  def build_generator(destination_root, options = {})
    RecordingStudioPublishedArticle::Generators::InstallGenerator.new(
      [],
      options,
  ...
  def test_mount_engine_uses_configured_mount_path
    ...
    assert_equal ["mount RecordingStudioPublishedArticle::Engine, at: \"/addons/recording\""], routes
  end
  ...
  def test_add_tailwind_source_does_not_duplicate_existing_entries
    with_temp_app do |dir|
      ...
      File.write(css_path, <<~CSS)
        ...
        @source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/recording_studio_published_article-*/app/views/**/*.erb";
  ...
  def test_install_guide_includes_migration_and_host_setup_steps
    ...
    assert_includes install_guide, "bin/rails generate recording_studio_published_article:migrations"
