...
class EngineTest < Minitest::Test
  def setup
    ...
    RecordingStudioPublishedArticle.instance_variable_set(:@configuration, RecordingStudioPublishedArticle::Configuration.new)
  end
  ...
  def test_before_and_after_initialize_initializers_run_hooks
    ...
    RecordingStudioPublishedArticle.configuration.hooks.before_initialize { |_engine| before_called = true }
    RecordingStudioPublishedArticle.configuration.hooks.after_initialize { |_engine| after_called = true }

    find_initializer("recording_studio_published_article.before_initialize").block.call(Object.new)
    find_initializer("recording_studio_published_article.after_initialize").block.call(Object.new)
  ...
  def test_load_config_merges_config_sources_and_runs_on_configuration_hook
    hook_called = false
    hook_payload = nil
    RecordingStudioPublishedArticle.configuration.hooks.on_configuration do |cfg|
      hook_called = true
      hook_payload = cfg
    end

    xcfg = Struct.new(:recording_studio_published_article).new({ article_parent_types: %w[Folder] })
    app_config = Struct.new(:x).new(xcfg)
    app = Struct.new(:config) do
      def config_for(_name)
        { article_parent_types: %w[Workspace] }
      end
    end.new(app_config)

    find_initializer("recording_studio_published_article.load_config").block.call(app)

    assert hook_called
    assert_equal RecordingStudioPublishedArticle.configuration, hook_payload
    assert_equal %w[Folder], RecordingStudioPublishedArticle.configuration.article_parent_types
  end

  def test_load_config_handles_errors_and_each_pair_fallback
    pair_config = Class.new do
      def each_pair
        yield(:article_parent_types, %w[Workspace])
      end
    end.new

    xcfg = Struct.new(:recording_studio_published_article).new(pair_config)
    app_config = Struct.new(:x).new(xcfg)
    ...
    assert_equal %w[Workspace], RecordingStudioPublishedArticle.configuration.article_parent_types
  end
  ...
  def test_load_config_swallow_each_pair_errors
    ...
    xcfg = Struct.new(:recording_studio_published_article).new(bad_pair_config)
    app_config = Struct.new(:x).new(xcfg)
    app = Struct.new(:config) do
      def config_for(_name)
        { article_parent_types: %w[Workspace] }
  ...
  def test_load_config_is_noop_without_config_sources
    ...
    assert_equal [], RecordingStudioPublishedArticle.configuration.article_parent_types
  end
  ...
  def test_apply_extension_initializers_register_active_support_on_load_callbacks
    ...
    RecordingStudioPublishedArticle::Engine.stub(:config, config_stub) do
      find_initializer("recording_studio_published_article.apply_model_extensions").block.call
      find_initializer("recording_studio_published_article.apply_controller_extensions").block.call
    end
  ...
  def test_model_extension_initializer_skips_abstract_models
    ...
    with_temporary_nested_constant(:ActiveRecord, :Base, active_record_base) do
      RecordingStudioPublishedArticle::Engine.stub(:apply_model_extensions, ->(model) { applied << model }) do
  ...
  def test_controller_extension_initializer_applies_all_controllers
    ...
    with_temporary_nested_constant(:ActionController, :Base, action_controller_base) do
      RecordingStudioPublishedArticle::Engine.stub(:apply_controller_extensions, lambda { |controller|
        applied << controller
      }) do
        to_prepare_blocks.first.call
      end
  ...
  def test_apply_model_extensions_adds_registered_methods_once
    ...
    RecordingStudioPublishedArticle.configuration.hooks.extend_model(:ExampleRecord) do
      def template_extension_method
        :applied
      end
    end

    RecordingStudioPublishedArticle::Engine.apply_model_extensions(model_class)
    RecordingStudioPublishedArticle::Engine.apply_model_extensions(model_class)
  ...
  def test_apply_controller_extensions_matches_demodulized_name
    ...
    RecordingStudioPublishedArticle.configuration.hooks.extend_controller(:DashboardController) do
      def template_controller_extension
        :applied
      end
    end

    RecordingStudioPublishedArticle::Engine.apply_controller_extensions(controller_class)
  ...
  def test_apply_extensions_flattens_compacts_and_tracks_identity
    ...
    RecordingStudioPublishedArticle::Engine.send(:apply_extensions, target, [nil, [extension, extension]])

    assert_equal :generated, target.new.generated_method
    applied = target.instance_variable_get(:@recording_studio_published_article_applied_extensions)
    assert_equal true, applied.compare_by_identity?
  end

  def test_apply_extensions_returns_without_target
    assert_nil RecordingStudioPublishedArticle::Engine.send(:apply_extensions, nil, [])
  end
  ...
  def test_extension_keys_for_includes_demodulized_name
    ...
    assert_equal expected_keys, RecordingStudioPublishedArticle::Engine.send(:extension_keys_for, namespaced)
  end
  ...
  def test_extension_keys_for_removes_duplicate_names
    ...
    assert_equal [:ReportsController], RecordingStudioPublishedArticle::Engine.send(:extension_keys_for, plain)
  end
  ...
  def find_initializer(name)
    RecordingStudioPublishedArticle::Engine.initializers.find { |initializer| initializer.name == name }
  end
end
