...
class RecordingStudioPublishedArticleTest < Minitest::Test
  def test_version_matches_release
    assert_equal "0.3.0", ::RecordingStudioPublishedArticle::VERSION
  end

  def test_engine_exists
    assert_kind_of Class, ::RecordingStudioPublishedArticle::Engine
  end

  def test_gemspec_pins_recording_studio_4_2
    gemspec = File.read(File.expand_path("../recording_studio_published_article.gemspec", __dir__))

    assert_includes gemspec, 'spec.add_dependency "recording_studio", "~> 4.2"'
    assert_includes gemspec, 'spec.add_dependency "recording_studio_publications", "~> 0.2"'
    ...
    assert_includes gemspec, "https://github.com/bowerbird-app/RecordingStudio_published_article"
  end

  def test_gemspec_excludes_cursor_config
    spec = Gem::Specification.load(File.expand_path("../recording_studio_published_article.gemspec", __dir__))
  ...
  def test_cursor_environment_is_repo_managed_without_snapshot
    ...
    assert_equal "recording-studio-published-article", json["name"]
  ...
  def test_dummy_gemfile_pins_verified_4x_github_tags
    ...
    assert_includes gemfile, 'github: "bowerbird-app/RecordingStudio_publications", tag: "v0.2.1"'
  ...
  def test_template_does_not_ship_copied_core_hooks_or_base_service
    ...
    refute File.exist?(File.expand_path("../lib/recording_studio_published_article/services/base_service.rb", __dir__))
    refute File.exist?(File.expand_path("../lib/recording_studio_published_article/services/example_service.rb",
                                        __dir__))
  end

  def test_example_capability_wraps_include_for_and_is_not_enabled_globally
    source = File.read(File.expand_path("../lib/recording_studio_published_article/capabilities/example.rb", __dir__))
  ...
  def test_recording_studio_keeps_strict_recordable_declarations_enabled
    ...
    assert_includes initializer_source, '"RecordingStudioPublishedArticle::PublishedArticle"'
  ...
  def test_dummy_readme_explains_dummy_app_purpose
    ...
    assert_includes readme_source, "This Rails app is the Published Articles host."
  ...
  def test_dummy_home_page_uses_demo_title_only
    ...
    assert_includes view_source, 'title: "Published articles"'
    assert_includes view_source, "RecordingStudioPublishedArticle::Articles::IndexComponent"
  ...
  def test_engine_does_not_ship_a_home_view
    view_path = File.expand_path("../app/views/recording_studio_published_article/home/index.html.erb", __dir__)
