...
module RecordingStudioPublishedArticle
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioPublishedArticle engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/recording_studio_published_article",
        desc: "Route prefix used when mounting the engine"
      )

      def mount_engine
        route %(mount RecordingStudioPublishedArticle::Engine, at: "#{options[:mount_path]}")
      end

      def copy_initializer
        template "recording_studio_published_article_initializer.rb",
                 "config/initializers/recording_studio_published_article.rb"
      end

      def add_yaml_config
        unless yes?(
          "Would you like to add `config/recording_studio_published_article.yml` " \
          "for environment-specific settings? [y/N]"
        )
          return
        end

        template "recording_studio_published_article.yml", "config/recording_studio_published_article.yml"
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioPublishedArticle and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioPublishedArticle and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioPublishedArticle engine views for Tailwind CSS */",
          missing_lines.first(2),
          "\n/* Include RecordingStudioPublishedArticle components for Tailwind CSS */",
          missing_lines[2, 2],
          "\n/* Include FlatPack component sources for Tailwind CSS */",
          missing_lines.drop(4)
      ...
      def tailwind_source_lines
        [
          '@source "../../vendor/bundle/**/recording_studio_published_article/app/views/**/*.erb";',
          '@source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/' \
          'recording_studio_published_article-*/app/views/**/*.erb";',
          '@source "../../vendor/bundle/**/recording_studio_published_article/app/components/**/*.{rb,erb}";',
          '@source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/' \
          'recording_studio_published_article-*/app/components/**/*.{rb,erb}";',
